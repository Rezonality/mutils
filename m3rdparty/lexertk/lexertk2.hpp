...
{
    return !is_whitespace(c) && !is_operator_char(c) && !is_letter(c) && !is_digit(c) && ('.' != c) && ('_' != c) && ('$' != c) && ('~' != c) && ('\'' != c);
}
...
struct token
{
    ...
    {
        if (
            (e_error == et) || (e_err_symbol == et) || (e_err_number == et) || (e_err_string == et))
        {
            type = e_error;
        }
        else
    ...
    {
        return (
            (e_error == type) || (e_err_symbol == type) || (e_err_number == type) || (e_err_string == type));
    }
...
public:
    ...
    lexertk::token _error;
    const lexertk::token& error_token() const
    {
        return _error;
    }
    ...
    inline bool current_token_is(token::token_type type)
    {
        return current_token().type == type;
    }

    inline std::string current_token_type_string()
    {
        return lexertk::token::to_str(current_token().type);

    }
    
    inline double current_token_as_number()
    {
        assert(current_token().type == token::e_number);
        return std::stod(current_token().value);
    }
    
    inline const std::string& current_token_as_string()
    {
        assert(current_token().type == token::e_string);
        return current_token().value;
    }

    inline const std::string& current_token_as_symbol()
    {
        assert(current_token().type == token::e_symbol);
        return current_token().value;
    }
...
private:
    ...
    inline void scan_operator()
    {
        ...
        if ('<' == *s_itr_)
            ...
            t.set_operator(token_t::e_or, s_itr_, s_itr_ + 1, base_itr_);
        else if ('.' == *s_itr_)
            t.set_operator(token_t::e_stop, s_itr_, s_itr_ + 1, base_itr_);
        else
    ...
    inline void scan_symbol()
    {
        ...
        while (
            (!is_end(s_itr_)) && (details::is_letter_or_digit(*s_itr_) || ((*s_itr_) == '_' || (*s_itr_) == ':')))
        {
            ++s_itr_;
        }
        token_t t;
    ...
    inline void scan_number()
    {
        ...
        if (!digit_found && dot_found)
        {
            t.set_operator(token::e_stop, begin, s_itr_, base_itr_);
        }
        else
        {
...
inline void dump(lexertk::generator& generator, std::ostringstream& str)
{
    for (std::size_t i = 0; i < generator.size(); ++i)
    {
        lexertk::token t = generator[i];
        str << "Token[" << i << "] @ " << t.position << "   " << t.to_str(t.type) << " -->  '" << t.value << "'" << std::endl;
    }
}
...
public:
    inline bool join(const lexertk::token& t0, const lexertk::token& t1, lexertk::token& t)
    {
        ...
        else if ((t0.type == lexertk::token::e_sub) && (t1.type == lexertk::token::e_gt))
        {
            t.type = lexertk::token::e_produces;
            t.value = "->";
            t.position = t0.position;
...
public:
    ...
    {
        if (
            !t.value.empty() && (lexertk::token::e_string != t.type) && (lexertk::token::e_symbol != t.type) && details::is_bracket(t.value[0]))
        {
            char c = t.value[0];
...
public:
    ...
    {
        if (
            (current_token().type != ttype) || !details::imatch(value, current_token().value))
        {
            return false;
        }
